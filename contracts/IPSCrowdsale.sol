...
import './IPSSmartToken.sol'; 


contract IPSCrowdsale is FinalizableCrowdsale {

    // ----------------------------------------------------------------------------------------------------
    //                                      Constants
    // ----------------------------------------------------------------------------------------------------
    // Max amount of known addresses of which will get IPS by 'Grant' method.
    //
    // grantees addresses will be IPS wallets addresses.
    // these wallets will contain IPS tokens that will be used for 2 purposes only -
    // 1. IPS tokens against raised fiat money
    // 2. IPS tokens for presale bonus.
    ...
    // wallets address for 60% of IPS allocation
    address public walletTeam;   //10% of the total number of IPS tokens will be allocated to the team
    address public walletOEM;       //10% of the total number of IPS tokens will be allocated to OEM’s, Operating System implementation, SDK developers 
    address public walletBounties;  //5% of the total number of IPS tokens will be allocated to professional fees and Bounties
    address public walletReserve;   //35% of the total number of IPS tokens will be allocated to IPS.DATA and as a reserve for the company to be used for future strategic plans for the created ecosystem
    ...
    function IPSCrowdsale(uint256 _startTime,
    uint256 _endTime,
    address _wallet,
    address _walletTeam,
    address _walletOEM,
    ...
    IPSSmartToken _ipsSmartToken,
    RefundVault _refundVault)
    public
    Crowdsale(_startTime, _endTime, EXCHANGE_RATE, _wallet, _ipsSmartToken) {
        ...
        require(_ipsSmartToken != address(0));
    ...
    // @return the rate in IPS per 1 ETH according to the time of the tx and the IPS.data pricing program.
    // @Override
    function getRate() public view returns (uint256) {
        if (now < (startTime.add(24 hours))) {return 100;} // 24-hours
        if (now < (startTime.add(2 days))) {return 95;}    // 2 day
        if (now < (startTime.add(3 days))) {return 90;}    // 3 day
        if (now < (startTime.add(4 days))) {return 85;}    // 4 day
        if (now < (startTime.add(5 days))) {return 80;}    // 5 day
        if (now < (startTime.add(6 days))) {return 75;}    // 6 day
        if (now < (startTime.add(7 days))) {return 70;}    // 7 day
        if (now < (startTime.add(8 days))) {return 65;}    // 8 day
        if (now < (startTime.add(9 days))) {return 60;}    // 9 day
        if (now < (startTime.add(10 days))) {return 55;}   // 10 day
        if (now < (startTime.add(11 days))) {return 50;}   // 11 day
        if (now < (startTime.add(12 days))) {return 45;}   // 12 day
        if (now < (startTime.add(13 days))) {return 40;}   // 13 day
        if (now < (startTime.add(14 days))) {return 35;}   // 13 day
        return rate;
    }
    ...
    function finalization() internal onlyOwner {
        ...
        // 10% of the total number of IPS tokens will be allocated to the team
        token.issue(walletTeam, newTotalSupply.mul(10).div(100));

        // 10% of the total number of IPS tokens will be allocated to OEM’s, Operating System implementation,
        // SDK developers 
        token.issue(walletOEM, newTotalSupply.mul(10).div(100));

        // 5% of the total number of IPS tokens will be allocated to professional fees and Bounties
        token.issue(walletBounties, newTotalSupply.mul(5).div(100));

        // 35% of the total number of IPS tokens will be allocated to APSDATA LABS,
