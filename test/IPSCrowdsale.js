...
//const IPSCrowdsale = artifacts.require('IPSCrowdsale.sol')
const IPSCrowdsale = artifacts.require('../contracts/IPSCrowdsale.sol')
const RefundVault = artifacts.require('../contracts/crowdsale/RefundVault.sol')
const IPSSmartToken = artifacts.require('IPSSmartToken.sol')

contract('IPSCrowdsale', function([_, investor, owner, wallet, walletTeam, walletOEM, walletBounties, walletReserve]) {
    ...
    beforeEach(async function() {
        ...
        this.token = await IPSSmartToken.new({from: owner});
        this.refundVault = await RefundVault.new(wallet, this.token.address,{from: owner});

        this.crowdsale = await IPSCrowdsale.new(this.startTime,
            this.endTime,
            wallet,
            walletTeam,
            walletOEM,
    ...
    describe('Grant tokens', function() {
        ...
        it('should allocate token as expected', async function() {
            ...
            let max_grantees = await this.crowdsale.MAX_TOKEN_GRANTEES()
            for (let i = 0; i < max_grantees; i++) {
                let address = "0x74f5baA7d79467bfFffe5d5F3Ac3957828Acbc23" + i
                await this.crowdsale.addUpdateGrantee(address, 100, {
                    from: owner
                });
            }
